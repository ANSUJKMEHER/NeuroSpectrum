...
class DifferentiableSimulationEngine(nn.Module):
    ...
    def _single_step_verlet(self, points: torch.Tensor, velocities: torch.Tensor, forces: torch.Tensor, gamma: torch.Tensor, is_inference: bool = False) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Velocity Verlet step for gradient checkpointing."""
        return step_velocity_verlet(
            points, velocities, forces, gamma, self.energy_model,
            dt=self.dt, L=self.L, damping=self.damping,
            use_knn=self.use_knn, k=self.k, is_inference=is_inference,
    ...
    def forward(self, points_init: torch.Tensor, gamma: torch.Tensor) -> torch.Tensor:
        ...
        if self.integrator == "verlet":
            velocities = torch.zeros_like(points)
            is_infer = not points.requires_grad
            forces, _ = compute_neural_force(
                points, gamma, self.energy_model, L=self.L,
                use_knn=self.use_knn, k=self.k, is_inference=is_infer
            )
            for step in range(active_steps):
                if self.use_checkpointing and points.requires_grad:
                    points, velocities, forces, _ = checkpoint(self._single_step_verlet, points, velocities, forces, gamma, use_reentrant=False)
                else:
                    points, velocities, forces, _ = self._single_step_verlet(points, velocities, forces, gamma, is_infer)
        else:
